...
node tests/tools/sandbox-e2e.js upgrade
...
- **Single-use reply codes (v2.2.0; supersedes single-use check-in tokens)**: each check-in email contains a unique 8-character code; replying with it resets the timer and marks the code used. The code IS the token — stored only as `sha256(code)` in `reply_codes` with kind (arming / checkin / ping-ack / warning-ack), user, `recipient_hash`, `ref`. Beneficiary kinds: exactly one live code per (user, kind, ref, recipient), issuing a new one retires the previous. Operator kinds (arming/checkin): **outstanding codes stay live** (at most 5, oldest retired) and **any successful check-in retires them all** — retiring on issue made a reply that crossed a check-in tick "expired" and fired the switch on a living operator in the 2026-09-18 live pass. Five wrong guesses retire a code and reissue. Nothing token-like lives in memory, so codes survive restarts. `GET /checkin/:token` and `GET /ack/:token` no longer exist; no email contains `APP_URL`.
- **Reply-by-email invariants (v2.2.0)** — `docs/reply-by-email.md` is the spec:
  ...
  - **Upgrade path** (`runReplyCodeMigration()`, once, keyed by `settings.migrated_reply_codes`, after recovery and before the reader starts): every armed switch gets a "Deploy was updated" check-in email through `issueCheckinEmail()` (live code, reissue stamp, `lastCheckinSentAt`) with its missed counter reset to zero; every sent-but-unanswered ping is re-sent with a code (opted-out recipients and non-armed switches skipped, serialized with the sweep). An operator without working IMAP is never touched beyond banner + alert.
  - The inbound reader starts only after restart recovery has re-armed every switch, or a reply that arrived during the restart would find no switch. `performCheckin(userEmail, switchData, {via})` is the one check-in implementation (only caller: the poller, via `reply`); `performAck(codeRow)` the one ack implementation. **There is no dashboard check-in** (removed after the live pass at Eric's call): one path, so the reply path is exercised every time; broken mail means fix it or abort, with the fail-safe hold in between.
